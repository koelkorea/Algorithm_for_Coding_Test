...
// 풀이 1) Node + Queue + 좌표배열 사용


// 좌표를 순차적으로 다루어야 하는 관계로 x,y를 쉽게 저장하고 불러올수 있는 클래스를 정의한다.
class Node {
	
	int x;
	int y;
	
	// 생성자를 통해 투입할 x,y값을 직관적으로 동시 저장가능
	...
	// getter를 통해 클래스에서 x,y를 쉽게 꺼내기도 가능하다. 
	public int getX() {
		return x;
	}
	...
	static int BFS(int x, int y){
		...
		while(!q.isEmpty()) {
			...
			for(int i = 0; i < 4; i++) {
				...
				// Node의 좌표(getter값)에 동서남북의 좌표를 순차적 입력(4번 수행되고, 순차적으로 queue에 입력)
				int new_x = node.getX() + move_x[i];
				int new_y = node.getY() + move_y[i];
				...
				if(maze[new_x][new_y] == 1) {
					...
					maze[new_x][new_y] = maze[node.getX()][node.getY()] + 1;
