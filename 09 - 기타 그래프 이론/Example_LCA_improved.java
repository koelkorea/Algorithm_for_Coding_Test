...
// [향상된 방법] : 반복 + 다이나믹 프로그래밍을 2차원으로 확대해서, 각 노드의 직부모 뿐 아니라, 2^i번째 부모(2진트리 그래프 구조하의 최대높이)에 대한 정보까지 기록함(2^0 = 1, 2^1 = 2므로, 최소 해당 원소의 직선조와 조선조까지는 확인가능)
//				 (그래프를 이루는 간선의 수가 100000개 이상인 경우 시간복잡성에 문제가 생기는부분 보완)
//				 -> 2진 트리 구조로 그래프가 짜여있다는 가정 필요

// [향상된 녀석의 최악의 시간복잡도] : M * logN(해당 2진트리 그래프의 높이) (거슬러 올라가는 속도가 2의 제곱으로 빨라졌기에 가능)
